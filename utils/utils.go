...
func Handler() {
	r := mux.NewRouter()
	r.HandleFunc(version+"country/{country_name}", handlers.CasesPerCountry).Queries("", "{begin_date-end_date}")
	r.HandleFunc(version+"country/{country_name}", handlers.CasesPerCountry)
	r.HandleFunc(version+"policy/{country_name}", handlers.CurrentStringencyLevel).Queries("", "{begin_date-end_date}")
